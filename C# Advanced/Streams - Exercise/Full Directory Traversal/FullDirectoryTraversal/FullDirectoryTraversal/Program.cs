...
namespace FullDirectoryTraversal
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> directories = Directory.GetDirectories(@"").ToList();

            for (int i = 0; i < directories.Count; i++)
            {
                string[] filesFromDirectory = Directory.GetFiles(directories.ElementAt(i));

                directories.AddRange(Directory.GetDirectories(directories.ElementAt(i)));
