...
namespace Sneaking
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int numberOfRowsInTheRoom = int.Parse(Console.ReadLine());
            char[][] room = new char[numberOfRowsInTheRoom][];
            int[] samPosition = new int[2];

            for (int row = 0; row < numberOfRowsInTheRoom; row++)
            {
                room[row] = Console.ReadLine().ToCharArray();
                if (room[row].Contains('S'))
                {
                    samPosition = new[] { row, Array.IndexOf(room[row], 'S') };
                }
            }
