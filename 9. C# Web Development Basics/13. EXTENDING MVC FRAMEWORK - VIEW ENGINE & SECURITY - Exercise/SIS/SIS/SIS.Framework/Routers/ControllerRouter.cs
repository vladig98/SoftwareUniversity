...
namespace SIS.Framework.Routers
{
    public class ControllerRouter : IHttpHandler
    {
        ...
        {
            if (action.GetCustomAttributes().Where(x => x is AuthorizeAttribute)
                .Cast<AuthorizeAttribute>().Any(x => !x.IsAuthorized(controller.Identity)))
            {
        ...
        private Controller GetController(string controllerName, IHttpRequest request)
        {
            if (controllerName != null)
            {
                ...
                var controller = (Controller)dependencyContainer.CreateInstance(controllerType);
        ...
        public IHttpResponse Handle(IHttpRequest request)
        {
            ...
            return Authorize(controller, method) ?? response;
        }
