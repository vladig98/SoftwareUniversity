    ...
    public class HttpRequest : IHttpRequest
    {
        ...
        private void ParseCookies()
        {
            if (Headers.ContainsHeader("Cookie"))
            {
                ...
                var cookieParams = header.Value.Split(new[] { "=", ";", " " }, StringSplitOptions.RemoveEmptyEntries);
                for (int i = 0; i < cookieParams.Length; i += 2)
                {
                    var cookie = new HttpCookie(cookieParams[i], cookieParams[i + 1]);
                    Cookies.Add(cookie);
                }
            }
        }
        ...
        private void ParseHeaders(string[] requestContent)
        {
            foreach (string header in requestContent)
            {
                if (!string.IsNullOrEmpty(header) && !string.IsNullOrWhiteSpace(header))
                {
                    ...
                    HttpHeader headerToInsert = new HttpHeader(headerTokens[0], headerTokens[1]);
